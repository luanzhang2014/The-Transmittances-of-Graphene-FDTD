...
ie=90*2;           %number of grid cells in x-direction
je=26*2;            %number of grid cells in y-direction
...
%dx=1532.0e-9; 
dt=dx/((2.0)^(1/2)*cc);   %time step
%dt=8.3391e-11;
%dx=0.05;
nmax=200000;         %total number of time steps
...
%for n=1:nmax
    ...
    %source(n)=100*(n*dt-t0)*exp(-b^2*(n*dt-t0)^2);
 %   source(n)=-dt*(10^(-10)*exp(-(((n-1)*dt-3*T)/T)^2)*(-2)*((n-1)*dt-3*T)/T/T)/epsz/dx/dx;
%end
%***********************************************************************
%     Field arrays
%***********************************************************************
...
T=1.0/(30*10^(13))/dt;
dstore1(n)=p(n);
%p(n)=10*exp(-((n-3*T)/T)^2);
p(n)=10*exp(-j*2*pi*30*10^(12)*n*dt);
ex(1,26)=ex(1,26)-0.5*(p(n)-dstore1(n));

ex(93:ie,26)=caex(93:ie,26).*ex(93:ie,26)+...
           cbex(93:ie,26).*(hz(93:ie,26)-hz(93:ie,25));
ex(1:45,36)=caex(1:45,36).*ex(1:45,36)+...
           cbex(1:45,36).*(hz(1:45,36)-hz(1:45,35));
ex(1:45,16)=caex(1:45,16).*ex(1:45,16)+...
           cbex(1:45,16).*(hz(1:45,16)-hz(1:45,15));

ex(:,2:15)=caex(:,2:15).*ex(:,2:15)+...
           cbex(:,2:15).*(hz(:,2:15)-hz(:,1:14));
ex(:,17:25)=caex(:,17:25).*ex(:,17:25)+...
           cbex(:,17:25).*(hz(:,17:25)-hz(:,16:24));
ex(:,27:35)=caex(:,27:35).*ex(:,27:35)+...
           cbex(:,27:35).*(hz(:,27:35)-hz(:,26:34));
ex(:,37:je)=caex(:,37:je).*ex(:,37:je)+...
           cbex(:,37:je).*(hz(:,37:je)-hz(:,36:je-1));
       
       
ex(1:92,26)=caex(1:92,26).*ex(1:92,26)+... 
    cbex(1:92,26).*(hz(1:92,26)-hz(1:92,25))-...
                                2*dt*Jx(1:92,26)/((epsz+epsz));
ex(46:ie,36)=caex(46:ie,36).*ex(46:ie,36)+... 
    cbex(46:ie,36).*(hz(46:ie,36)-hz(46:ie,35))-...
                                2*dt*Jx(46:ie,36)/((epsz+epsz));
ex(46:ie,16)=caex(46:ie,16).*ex(46:ie,16)+... 
    cbex(46:ie,16).*(hz(46:ie,16)-hz(46:ie,15))-...
                                2*dt*Jx(46:ie,16)/((epsz+epsz));
...
Jx(1:92,26)=(2*tao-dt)*Jx(1:92,26)/(2*tao+dt)+2*A*dt*ex(1:92,26)/(2*tao+dt)/dx;
Jx(46:ie,36)=(2*tao-dt)*Jx(46:ie,36)/(2*tao+dt)+2*A*dt*ex(46:ie,36)/(2*tao+dt)/dx;
Jx(46:ie,16)=(2*tao-dt)*Jx(46:ie,16)/(2*tao+dt)+2*A*dt*ex(46:ie,16)/(2*tao+dt)/dx;
...
e(:,:)=ex(1:ie,1:je).*conj(ex(1:ie,1:je))+ey(1:ie,1:je).*conj(ey(1:ie,1:je));
%***********************************************************************
...
subplot('position',[0.15 0.45 0.7 0.45]),imagesc(e');
shading flat;
caxis([-80.0 80.0]);
axis([1 ie 1 jb]);
colorbar;
axis image; axis xy
axis off;
title(['E at time step = ',timestep]);

%subplot('position',[0.15 0.30 0.7 0.35]),imagesc(ey');
%shading flat;
%caxis([-80.0 80.0]);
%axis([1 ib 1 je]);
%colorbar;
...
subplot('position',[0.15 0.10 0.7 0.25]),imagesc(ey');
shading flat;
caxis([-0.2 0.2]);
axis([1 ie 1 je]);
colorbar;
