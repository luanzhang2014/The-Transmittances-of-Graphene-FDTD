%***********************************************************************
%     The transmittance of graphene 2-D FDTD TE code with PML absorbing boundary conditions
%***********************************************************************
%
...
ie=35;           %number of grid cells in x-direction
je=30;            %number of grid cells in y-direction
...
dx=10.0e-9;        %space increment of square lattice
%dx=1532.0e-9; 
dt=dx/((2.0)^(1/2)*cc);   %time step
%dt=8.3391e-11;
%dx=0.05;
nmax=70000;         %total number of time steps
...
p(n)=0.2*exp(-j*2*pi*30*10^(12)*n*dt);
ex(1,15)=ex(1,15)-0.5*(p(n)-dstore1(n));

ex(:,2:14)=caex(:,2:14).*ex(:,2:14)+...
           cbex(:,2:14).*(hz(:,2:14)-hz(:,1:13));
ex(:,16:je)=caex(:,16:je).*ex(:,16:je)+...
           cbex(:,16:je).*(hz(:,16:je)-hz(:,15:je-1));
 
       
ex(:,15)=caex(:,15).*ex(:,15)+... 
    cbex(:,15).*(hz(:,15)-hz(:,14))-...
                                2*dt*Jx(:,15)/((epsz+epsz));
...
Jx(:,15)=(2*tao-dt)*Jx(:,15)/(2*tao+dt)+2*A*dt*ex(:,15)/(2*tao+dt)/dx;
...
subplot('position',[0.15 0.45 0.7 0.45]),imagesc(ex');
shading flat;
caxis([-80.0 80.0]);
axis([1 ie 1 jb]);
colorbar;
